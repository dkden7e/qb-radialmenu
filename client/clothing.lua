...
local Extras = {
	["Shirt"] = {
		Drawable = 11,
		Table = {
			Standalone = true, male = 252, female = 101,
			Extra = {
	...
	["Pants"] = {
		Drawable = 4,
		Table = {Standalone = true, male = 61, female = 56},
...
function ToggleClothing(whic, extra)
	...
	if which ~= "Mask" then
		if not Gender then Notify(traslatear("info.wrong_ped")) return false end -- We cancel the command here if the person is not using a multiplayer model.
	end
	...
	if not Toggle.Table.Standalone then -- "Standalone" is for things that dont require a variant, like the shoes just need to be switched to a specific drawable. Looking back at this i should have planned ahead, but it all works so, meh!
		...
		Notify(traslatear("info.no_variants")) return
	else
		if not LastEquipped[which] then
	...
	Notify(traslatear("info.already_wearing")) return false
end
...
function ToggleProps(whic)
	...
	if not Prop.Variants then
		...
		Notify(traslatear("info.nothing_to_remove")) return false
	else
		local Gender = IsMpPed(Ped)
		if not Gender then Notify(traslatear("info.wrong_ped")) return false end -- We dont really allow for variants on ped models, Its possible, but im pretty sure 95% of ped models dont really have variants.
		...
		Notify(traslatear("info.no_variants")) return false
	end
end
...
function Notify(message, color) -- However you want your notifications to be shown, you can switch it up here.
	ESX.ShowHelpNotification(message, color)
end
